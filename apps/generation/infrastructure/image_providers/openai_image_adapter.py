"""OpenAI gpt-image-2 image provider adapter.
...
class OpenAIImageAdapter:
    """Adapter for OpenAI gpt-image-2.
    ...
    def _get_api_key(self) -> str:
        ...
        if not api_key:
            raise OperationError(
                "OpenAIImageAdapter._get_api_key",
        ...
        **kwargs,
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                logger.info(f"Calling OpenAI API: {self.full_url}")
            ...
            logger.error(f"Unexpected error in OpenAIImageAdapter.generate_image: {e}")
            raise OperationError(
                "OpenAIImageAdapter.generate_image",
    ...
    def _calculate_cost(self, prompt: str, response_data: dict[str, Any]) -> CostMetadata:
        ...
        # Cost per image (OpenAI DALL-E 3: approximately $0.04 per image for 1024x1024)
        cost_per_image = 0.04
