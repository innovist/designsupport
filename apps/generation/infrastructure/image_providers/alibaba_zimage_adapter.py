"""Alibaba Cloud z-image-turbo image provider adapter.
...
class AlibabaZImageAdapter:
    """Adapter for Alibaba Cloud z-image-turbo model.
    ...
    def __init__(self, provider: ModelProvider, model: ModelCatalog):
        ...
        self.base_url = provider.base_url or "https://dashscope-intl.aliyuncs.com/compatible-mode/v1"
        ...
        if self.base_url.endswith(self.endpoint_path):
            ...
            self.full_url = f"{self.base_url}{self.endpoint_path[1:]}"
        else:
    ...
    def _get_api_key(self) -> str:
        ...
        if not api_key:
            raise OperationError(
                "AlibabaZImageAdapter._get_api_key",
    ...
    async def generate_image(
        ...
        **kwargs,
        ...
        """Generate an image using Alibaba z-image-turbo.
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                # Prepare request payload for Alibaba API
                payload = {
                ...
                logger.info(f"Calling Alibaba API: {self.full_url}")
                ...
                if response.status_code >= 400:
                    ...
                    raise OperationError(
                        "AlibabaZImageAdapter.generate_image",
            ...
            logger.error("Alibaba API request timed out")
            raise OperationError(
                "AlibabaZImageAdapter.generate_image",
            ...
            logger.error(f"Unexpected error in AlibabaZImageAdapter.generate_image: {e}")
            raise OperationError(
                "AlibabaZImageAdapter.generate_image",
    ...
    def _calculate_cost(self, prompt: str, response_data: dict[str, Any]) -> CostMetadata:
        ...
        # Cost per image (Alibaba z-image-turbo: approximately $0.015 per image)
    ...
    def get_model_key(self) -> str:
        ...
        Returns:
            Model identifier (e.g., "z-image-turbo")
        """
