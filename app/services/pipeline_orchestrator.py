...
# @MX:WARN: [AUTO] Complex async orchestration with multiple concurrent AI model calls
# @MX:REASON: Manages 8-step pipeline with parallel async operations, error recovery, and progress tracking

from datetime import datetime
from typing import Dict, Any, List, Optional, Callable

from app.core.config import get_settings, get_local_now
...
class FashionPipelineOrchestrator:
    """8단계 패션 분석 파이프라인 오케스트레이터 (AI 직접 조사 포함)"""
    ...
    def __init__(self):
        ...
        self.ai_research_service = AIResearchService()
    ...
    async def run_complete_pipeline(
        ...
        # 3.5단계: AI 직접 조사
        ai_research = await self._conduct_ai_research(session_data, keywords, progress_cb)
        analysis = await self._analyze_trends(session_data, crawled, keywords, progress_cb, ai_research)
    ...
    async def _glm_keyword_fallback(self, system_prompt: str, user_content: str) -> Dict[str, Any]:
        ...
        if not response or not hasattr(response, 'text') or not response.text:
            logger.error("GLM keyword fallback returned empty response")
            raise ValueError("Failed to get keywords from GLM")
        return parse_json(response.text)

    # @MX:ANCHOR: [AUTO] Keyword extraction - core NLP analysis for search queries
    # @MX:REASON: Called from 5+ locations as the primary keyword generation interface
    async def _extract_keywords(
        ...
        system_prompt = ("당신은 패션 트렌드 리서치 키워드 생성기입니다. 입력과 필터를 바탕으로 실제 검색에 사용할 키워드/검색어를 정확히 5개 생성하세요. "
                         ...
                         "키워드는 한국어 중심의 최대 3단어 검색어로 중복 없이 작성하세요. 길이가 너무 길면 검색 효율이 떨어집니다. "
                         "JSON 형식만 응답: {\"keywords\": [\"키워드1\", \"키워드2\", \"키워드3\", \"키워드4\", \"키워드5\"]}")
        ...
        if len(keywords) > 5:
            keywords = keywords[:5]
            logger.info(f"키워드를 상위 5개로 제한했습니다: {keywords}")
        ...
        for kw in keywords:
            ...
            if word_count <= 3:
                ...
                # 3단어 초과 시 상위 3단어만 사용
                words = kw.split()
                valid_keywords.append(" ".join(words[:3]))
                logger.warning(f"키워드가 3단어 초과하여 잘림: {kw} -> {valid_keywords[-1]}")

        keywords = valid_keywords
        if not keywords:
            raise ValueError("키워드 추출 실패: 유효한 키워드가 없습니다")
