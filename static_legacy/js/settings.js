...
class SettingsManager {
    ...
    initializeEventListeners() {
        ...
        document.getElementById('reset-keys').addEventListener('click', () => {
            if (confirm(_t('settings.confirmations.resetKeys'))) {
                this.resetApiKeys();
            }
        });
        ...
        document.getElementById('reset-system').addEventListener('click', () => {
            if (confirm(_t('settings.confirmations.resetSystem'))) {
    ...
    async testApiConnections() {
        ...
        try {
            ...
            uiManager.showError(_t('errors.unauthorized'));
            return;
        }
    ...
    async ensureSession() {
        ...
        const response = await fetch('/api/v1/settings/login', {
            method: 'POST',
